...
public class Logger {
	 ...
	 * @param s
	 */
	synchronized public static void log(String s,HttpServletRequest request) {
		bb.delete(0, bb.length());
		bb.append(time());
		//获取线程信息
		String threadName = Thread.currentThread().getName();
		bb.append(" ["+threadName+"] ");
		bb.append(marsk);
		bb.append(" - ");
		// 获取调用的位置
		...
		bb.append("line:"+stack[2].getLineNumber());
		bb.append(" - ");
		bb.append(s);
		bb.append(StringUtils.getSingleVerticalLine());
		bb.append(ChannelTemplate.getLogParam(request));
		out(bb.toString());
	}

	public  void debug(String s,HttpServletRequest request) {
	...
	public  void warn(String s,HttpServletRequest request) {
		if (level <= WARN_LEVEL) {
			marsk = "WARN";
			log(s,request);
		}
	...
	public  void error(String s,HttpServletRequest request) {
	...
	public  void fatal(String s,HttpServletRequest request) {
	...
	protected static BufferedWriter getWriter() {
		...
		File fi = new File(logdir + File.separator + new SimpleDateFormat("yyyy年").format(data) + File.separator
				+ new SimpleDateFormat("MM月").format(data) + File.separator + new SimpleDateFormat("dd").format(data)
				+ "日.log");
		try {
			if (!fi.exists()) {
	 ...
	 * 
	 * @param out_put_level
	 *            日志输出等级,只输出较高等级的 debug < info < warn < error < fatal
	 * @param out2where
	 *            输出到哪里 OUT_CONSOLE仅仅控制台输出,OUT_FILE仅仅输出到文件,OUT_BOTH均输出
	 * @param log_dir
	 *            日志保存到哪里
	 */
